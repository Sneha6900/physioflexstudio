...
function SpecialistsPage() {
  ...
  return (
    <FlowShell step={4}>
      ...
      {/* filters */}
      <div className="mt-8 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {specs.map((s) => (
            <button
              ...
              className={cn(
                ...
                spec === s ? "bg-accent text-charcoal" : "bg-white/5 text-white/65 hover:text-offwhite",
              )}
            >
              {s}
            </button>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-white/40">Sort by</span>
          {sorts.map((s) => (
            <button
              key={s}
              onClick={() => setSort(s)}
              className={cn(
                ...
                sort === s ? "border-accent bg-accent/10 text-accent" : "border-white/15 text-white/50",
              )}
            >
              {s}
            </button>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((e) => (
          <motion.div
            layout
            key={e.id}
            className="group flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] transition-all hover:-translate-y-1 hover:border-accent/40"
          >
            ...
            <div className="flex flex-1 flex-col p-5">
              <h3 className="font-display text-lg font-bold text-offwhite">{e.name}</h3>
              <p className="text-sm text-accent">{e.spec}</p>
              <div className="mt-4 flex flex-wrap gap-3 text-xs font-medium text-white/55">
