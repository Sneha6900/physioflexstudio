...
export function Hero() {
  ...
  return (
    ...
    >
      {/* Background image — responsive focal point via CSS media queries */}
      <motion.div className="hero-bg-wrap z-0" style={{ y: bgY }} aria-hidden>
        <img
          src={heroBg}
          alt=""
          fetchPriority="high"
      ...
      {/* Gradient overlay — mobile variant reveals subjects on the right */}
      <motion.div
        className="hero-overlay z-[1]"
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col justify-center px-4 pb-4 pt-[calc(var(--site-nav-height)+0.25rem)] sm:px-6 sm:pb-6 sm:pt-[calc(var(--site-nav-height)+0.5rem)] lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,15.5rem)] lg:items-center lg:gap-8 lg:pb-8 lg:pt-[calc(var(--site-nav-height)+0.75rem)] xl:grid-cols-[minmax(0,1fr)_minmax(0,17.5rem)]">
        ...
        <div className="min-w-0">
          ...
          <motion.p
            ...
            className="type-body mt-[var(--space-heading-gap)] max-w-md text-white/80"
          >
          ...
          <motion.div
            ...
            className="mt-[var(--space-stack-sm)] flex flex-col gap-2 sm:flex-row sm:gap-2.5"
          >
            <LeavingHomeLink
              to="/assessment/start"
